...
unsafe impl Send for Ewdg {}
unsafe impl Sync for Ewdg {}
impl Ewdg {
    #[inline(always)]
...
pub mod regs {
    ...
    impl defmt::Format for Ctrl1 {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Ctrl1 {{ parity_fail_int_en: {=bool:?}, parity_fail_rst_en: {=bool:?}, unl_ctl_fail_int_en: {=bool:?}, unl_ctl_fail_rst_en: {=bool:?}, ctl_vio_int_en: {=bool:?}, ctl_vio_rst_en: {=bool:?}, ot_rst_en: {=bool:?}, unl_ref_fail_int_en: {=bool:?}, unl_ref_fail_rst_en: {=bool:?}, ref_fail_int_en: {=bool:?}, ref_fail_rst_en: {=bool:?} }}" , self . parity_fail_int_en () , self . parity_fail_rst_en () , self . unl_ctl_fail_int_en () , self . unl_ctl_fail_rst_en () , self . ctl_vio_int_en () , self . ctl_vio_rst_en () , self . ot_rst_en () , self . unl_ref_fail_int_en () , self . unl_ref_fail_rst_en () , self . ref_fail_int_en () , self . ref_fail_rst_en ())
        }
    }
    #[doc = "wdog timeout reset counter value."]
    #[repr(transparent)]
    ...
    impl defmt::Format for WdtStatus {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "WdtStatus {{ ref_vio: {=bool:?}, ref_unl_fail: {=bool:?}, ctl_vio: {=bool:?}, ctl_unl_fail: {=bool:?}, ot_rst: {=bool:?}, parity_error: {=bool:?} }}" , self . ref_vio () , self . ref_unl_fail () , self . ctl_vio () , self . ctl_unl_fail () , self . ot_rst () , self . parity_error ())
        }
    }
}
