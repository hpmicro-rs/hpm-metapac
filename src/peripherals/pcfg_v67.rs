...
impl Pcfg {
    ...
    #[inline(always)]
    pub const fn debug_stop(self) -> crate::common::Reg<regs::DebugStop, crate::common::RW> {
...
pub mod regs {
    ...
    impl DebugStop {
        ...
        #[must_use]
        #[inline(always)]
        pub const fn cpu0(&self) -> bool {
            let val = (self.0 >> 0usize) & 0x01;
            val != 0
        }
        #[doc = "Stop peripheral when CPU0 enter debug mode 0: peripheral keep running when CPU0 in debug mode 1: peripheral enter debug mode when CPU0 enter debug."]
        #[inline(always)]
        pub const fn set_cpu0(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 0usize)) | (((val as u32) & 0x01) << 0usize);
        }
    ...
    impl Default for DebugStop {
        #[inline(always)]
        fn default() -> DebugStop {
            DebugStop(0)
        }
    }
    impl core::fmt::Debug for DebugStop {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("DebugStop")
                .field("cpu0", &self.cpu0())
                .field("cpu1", &self.cpu1())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for DebugStop {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "DebugStop {{ cpu0: {=bool:?}, cpu1: {=bool:?} }}",
                self.cpu0(),
                self.cpu1()
            )
        }
    ...
    impl ScgCtrl {
        #[doc = "control whether clock being gated during PMIC low power flow, 2 bits for each peripheral 00,01: reserved 10: clock is always off 11: clock is always on bit0-1: fuse bit2-3: sram bit4-5: vad bit6-7:gpio bit8-9:ioc bit10-11: timer bit12-13:wdog bit14-15:uart bit16-17:debug."]
        #[must_use]
        #[inline(always)]
