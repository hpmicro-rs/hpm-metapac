...
pub mod regs {
    ...
    impl Idmisc {
        #[doc = "DMA Idle Flag 0 - DMA is busy 1 - DMA is dile."]
        #[must_use]
        #[inline(always)]
        pub const fn idle_flag(&self) -> bool {
            let val = (self.0 >> 15usize) & 0x01;
            val != 0
        }
        #[doc = "DMA Idle Flag 0 - DMA is busy 1 - DMA is dile."]
        #[inline(always)]
        pub const fn set_idle_flag(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 15usize)) | (((val as u32) & 0x01) << 15usize);
        }
    }
    ...
    impl core::fmt::Debug for Idmisc {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Idmisc")
                .field("idle_flag", &self.idle_flag())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Idmisc {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Idmisc {{ idle_flag: {=bool:?} }}", self.idle_flag())
        }
    }
    #[doc = "Interrupt Status Register."]
    #[repr(transparent)]
