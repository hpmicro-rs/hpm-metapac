...
pub mod regs {
    ...
    impl defmt::Format for Gcr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Gcr {{ swfrc: {=bool:?}, frctime: {=u8:?}, timerreset: {=bool:?}, xrldsyncen: {=bool:?}, faultclr: {=bool:?}, cen: {=bool:?}, rldsyncen: {=bool:?}, faultexpol: {=u8:?}, faulte0en: {=bool:?}, faulte1en: {=bool:?}, faultrechwsel: {=u8:?}, faultrecedg: {=bool:?}, cmpshdwsel: {=u8:?}, hwshdwedg: {=bool:?}, frcpol: {=bool:?}, debugfault: {=bool:?}, faulti0en: {=bool:?}, faulti1en: {=bool:?}, faulti2en: {=bool:?}, faulti3en: {=bool:?} }}" , self . swfrc () , self . frctime () , self . timerreset () , self . xrldsyncen () , self . faultclr () , self . cen () , self . rldsyncen () , self . faultexpol () , self . faulte0en () , self . faulte1en () , self . faultrechwsel () , self . faultrecedg () , self . cmpshdwsel () , self . hwshdwedg () , self . frcpol () , self . debugfault () , self . faulti0en () , self . faulti1en () , self . faulti2en () , self . faulti3en ())
        }
    }
    #[doc = "Interrupt request enable register."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Pwmcfg {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Pwmcfg {{ deadarea: {=u32:?}, pair: {=bool:?}, frcsrcsel: {=bool:?}, faultrectime: {=u8:?}, faultmode: {=u8:?}, frcshdwupt: {:?}, oen: {=bool:?} }}" , self . deadarea () , self . pair () , self . frcsrcsel () , self . faultrectime () , self . faultmode () , self . frcshdwupt () , self . oen ())
        }
    }
    #[doc = "Counter reload register."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Shcr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Shcr {{ shlken: {=bool:?}, cntshdwupt: {:?}, cntshdwsel: {=u8:?}, frcshdwsel: {=u8:?} }}" , self . shlken () , self . cntshdwupt () , self . cntshdwsel () , self . frcshdwsel ())
        }
    }
    #[doc = "Shadow registers lock register."]
    #[repr(transparent)]
