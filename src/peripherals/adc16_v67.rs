...
pub mod regs {
    ...
    impl Adc16Config0 {
        ...
        #[doc = "set to enable temp sensor."]
        #[must_use]
        #[inline(always)]
        pub const fn tempsns_en(&self) -> bool {
            let val = (self.0 >> 25usize) & 0x01;
            val != 0
        }
        #[doc = "set to enable temp sensor."]
        #[inline(always)]
        pub const fn set_tempsns_en(&mut self, val: bool) {
    ...
    impl core::fmt::Debug for Adc16Config0 {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Adc16Config0")
                ...
                .field("tempsns_en", &self.tempsns_en())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Adc16Config0 {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Adc16Config0 {{ conv_param: {=u16:?}, preempt_en: {=bool:?}, cal_avg_cfg: {=u8:?}, bandgap_en: {=bool:?}, reg_en: {=bool:?}, tempsns_en: {=bool:?} }}" , self . conv_param () , self . preempt_en () , self . cal_avg_cfg () , self . bandgap_en () , self . reg_en () , self . tempsns_en ())
        }
    }
    #[doc = "No description available."]
    #[repr(transparent)]
    ...
    impl defmt::Format for AnaCtrl0 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "AnaCtrl0 {{ startcal: {=bool:?}, adc_clk_on: {=bool:?} }}",
                self.startcal(),
                self.adc_clk_on()
            )
        }
    ...
    impl defmt::Format for IntEn {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "IntEn {{ wdog[0]: {=bool:?}, wdog[1]: {=bool:?}, wdog[2]: {=bool:?}, wdog[3]: {=bool:?}, wdog[4]: {=bool:?}, wdog[5]: {=bool:?}, wdog[6]: {=bool:?}, wdog[7]: {=bool:?}, wdog[8]: {=bool:?}, wdog[9]: {=bool:?}, wdog[10]: {=bool:?}, wdog[11]: {=bool:?}, wdog[12]: {=bool:?}, wdog[13]: {=bool:?}, wdog[14]: {=bool:?}, wdog[15]: {=bool:?}, ahb_err: {=bool:?}, dma_fifo_full: {=bool:?}, seq_cvc: {=bool:?}, seq_cmpt: {=bool:?}, seq_dmaabt: {=bool:?}, seq_hw_cflct: {=bool:?}, seq_sw_cflct: {=bool:?}, read_cflct: {=bool:?}, trig_hw_cflct: {=bool:?}, trig_sw_cflct: {=bool:?}, trig_cmpt: {=bool:?} }}" , self . wdog (0usize) , self . wdog (1usize) , self . wdog (2usize) , self . wdog (3usize) , self . wdog (4usize) , self . wdog (5usize) , self . wdog (6usize) , self . wdog (7usize) , self . wdog (8usize) , self . wdog (9usize) , self . wdog (10usize) , self . wdog (11usize) , self . wdog (12usize) , self . wdog (13usize) , self . wdog (14usize) , self . wdog (15usize) , self . ahb_err () , self . dma_fifo_full () , self . seq_cvc () , self . seq_cmpt () , self . seq_dmaabt () , self . seq_hw_cflct () , self . seq_sw_cflct () , self . read_cflct () , self . trig_hw_cflct () , self . trig_sw_cflct () , self . trig_cmpt ())
        }
    }
    #[doc = "No description available."]
    #[repr(transparent)]
