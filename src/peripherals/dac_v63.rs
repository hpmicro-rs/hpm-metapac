...
pub mod regs {
    ...
    impl defmt::Format for DmaEn {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "DmaEn {{ buf0_cmpt: {=bool:?}, buf1_cmpt: {=bool:?} }}",
                self.buf0_cmpt(),
                self.buf1_cmpt()
            )
        }
    ...
    impl defmt::Format for IrqSts {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "IrqSts {{ buf0_cmpt: {=bool:?}, buf1_cmpt: {=bool:?}, fifo_empty: {=bool:?}, ahb_error: {=bool:?} }}" , self . buf0_cmpt () , self . buf1_cmpt () , self . fifo_empty () , self . ahb_error ())
        }
    }
    #[doc = "No description available."]
    #[repr(transparent)]
