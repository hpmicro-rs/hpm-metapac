...
pub mod regs {
    ...
    impl defmt::Format for Ctrl {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Ctrl {{ enable: {=bool:?}, inttcmask: {=bool:?}, interrmask: {=bool:?}, intabtmask: {=bool:?}, inthalfcntmask: {=bool:?}, dstaddrctrl: {:?}, srcaddrctrl: {:?}, dstmode: {:?}, srcmode: {:?}, dstwidth: {:?}, srcwidth: {:?}, srcburstsize: {=u8:?}, burstopt: {=bool:?}, priority: {=bool:?}, handshakeopt: {=bool:?}, infiniteloop: {=bool:?} }}" , self . enable () , self . inttcmask () , self . interrmask () , self . intabtmask () , self . inthalfcntmask () , self . dstaddrctrl () , self . srcaddrctrl () , self . dstmode () , self . srcmode () , self . dstwidth () , self . srcwidth () , self . srcburstsize () , self . burstopt () , self . priority () , self . handshakeopt () , self . infiniteloop ())
        }
    }
    #[doc = "DMAC Configuration Register."]
    #[repr(transparent)]
