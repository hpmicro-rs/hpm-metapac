...
is used to scale the alpha value embedded in the stream Others: Reserved."]
    ...
    impl defmt::Format for OutCtrl {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "OutCtrl {{ format: {=u8:?}, ablend_mode: {=u8:?}, srcalpha_op: {=u8:?}, dstalpha_op: {=u8:?}, srcalpha: {=u8:?}, dstalpha: {=u8:?} }}" , self . format () , self . ablend_mode () , self . srcalpha_op () , self . dstalpha_op () , self . srcalpha () , self . dstalpha ())
        }
    }
    #[doc = "Output Lower Right Corner Register."]
    #[repr(transparent)]
