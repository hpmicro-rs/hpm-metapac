...
pub mod regs {
    ...
    impl defmt::Format for Fcr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Fcr {{ fifoe: {=bool:?}, rfiforst: {=bool:?}, tfiforst: {=bool:?}, dmae: {=bool:?}, tfifot: {:?}, rfifot: {:?} }}" , self . fifoe () , self . rfiforst () , self . tfiforst () , self . dmae () , self . tfifot () , self . rfifot ())
        }
    }
    #[doc = "FIFO Control Register config."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Fcrr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Fcrr {{ fifoe: {=bool:?}, rfiforst: {=bool:?}, tfiforst: {=bool:?}, dmae: {=bool:?}, tfifot: {=u8:?}, rfifot: {=u8:?} }}" , self . fifoe () , self . rfiforst () , self . tfiforst () , self . dmae () , self . tfifot () , self . rfifot ())
        }
    }
    #[doc = "GPR Register."]
    #[repr(transparent)]
    ...
    impl IdleCfg {
        ...
        #[doc = "IDLE Detection Condition 0 - Treat as idle if RX pin is logic one 1 - Treat as idle if UART state machine state is idle."]
        #[inline(always)]
        pub const fn set_rx_idle_cond(&mut self, val: bool) {
    ...
    impl defmt::Format for IdleCfg {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "IdleCfg {{ rx_idle_thr: {=u8:?}, rx_idle_en: {=bool:?}, rx_idle_cond: {=bool:?}, rxen: {=bool:?} }}" , self . rx_idle_thr () , self . rx_idle_en () , self . rx_idle_cond () , self . rxen ())
        }
    }
    #[doc = "Interrupt Enable Register (when DLAB = 0)."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Ier {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Ier {{ erbi: {=bool:?}, ethei: {=bool:?}, elsi: {=bool:?}, emsi: {=bool:?}, erxidle: {=bool:?} }}" , self . erbi () , self . ethei () , self . elsi () , self . emsi () , self . erxidle ())
        }
    }
    #[doc = "Interrupt Identification Register."]
    #[repr(transparent)]
    ...
    impl defmt::Format for Lsr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Lsr {{ dr: {=bool:?}, oe: {=bool:?}, pe: {=bool:?}, fe: {=bool:?}, lbreak: {=bool:?}, thre: {=bool:?}, temt: {=bool:?}, errf: {=bool:?} }}" , self . dr () , self . oe () , self . pe () , self . fe () , self . lbreak () , self . thre () , self . temt () , self . errf ())
        }
    }
    #[doc = "Modem Control Register (."]
    #[repr(transparent)]
