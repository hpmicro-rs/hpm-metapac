...
pub mod regs {
    ...
    impl defmt::Format for Cr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Cr {{ capmode: {:?}, dbgpause: {=bool:?}, swsyncien: {=bool:?}, dmaen: {=bool:?}, dmasel: {:?}, cmpen: {=bool:?}, cmpinit: {=bool:?}, cen: {=bool:?}, synciren: {=bool:?}, syncifen: {=bool:?}, syncflw: {=bool:?}, cntrst: {=bool:?}, cntupt: {=bool:?} }}" , self . capmode () , self . dbgpause () , self . swsyncien () , self . dmaen () , self . dmasel () , self . cmpen () , self . cmpinit () , self . cen () , self . synciren () , self . syncifen () , self . syncflw () , self . cntrst () , self . cntupt ())
        }
    }
    #[doc = "Global control register."]
    #[repr(transparent)]
