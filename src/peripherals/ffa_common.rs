...
pub mod regs {
    ...
    impl defmt::Format for Status {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Status {{ op_cmd_done: {=bool:?}, nxt_cmd_rd_done: {=bool:?}, rd_err: {=bool:?}, rd_nxt_err: {=bool:?}, wr_err: {=bool:?}, fft_ov: {=bool:?}, fir_ov: {=bool:?} }}" , self . op_cmd_done () , self . nxt_cmd_rd_done () , self . rd_err () , self . rd_nxt_err () , self . wr_err () , self . fft_ov () , self . fir_ov ())
        }
    }
}
