...
pub mod regs {
    ...
    impl defmt::Format for Gcr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Gcr {{ swfrc: {=bool:?}, frctime: {=u8:?}, xrldsyncen: {=bool:?}, faultclr: {=bool:?}, cen: {=bool:?}, rldsyncen: {=bool:?}, faultexpol: {=u8:?}, faulte0en: {=bool:?}, faulte1en: {=bool:?}, faultrechwsel: {=u8:?}, faultrecedg: {=bool:?}, cmpshdwsel: {=u8:?}, hwshdwedg: {=bool:?}, frcpol: {=bool:?}, debugfault: {=bool:?}, faulti0en: {=bool:?}, faulti1en: {=bool:?}, faulti2en: {=bool:?}, faulti3en: {=bool:?} }}" , self . swfrc () , self . frctime () , self . xrldsyncen () , self . faultclr () , self . cen () , self . rldsyncen () , self . faultexpol () , self . faulte0en () , self . faulte1en () , self . faultrechwsel () , self . faultrecedg () , self . cmpshdwsel () , self . hwshdwedg () , self . frcpol () , self . debugfault () , self . faulti0en () , self . faulti1en () , self . faulti2en () , self . faulti3en ())
        }
    }
    #[doc = "Interrupt request enable register."]
    #[repr(transparent)]
