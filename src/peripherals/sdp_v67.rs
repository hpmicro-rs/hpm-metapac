...
from OTP for AES128; not valid for AES256. Other values, reserved."]
    ...
    impl defmt::Format for Sdpcr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Sdpcr {{ inten: {=bool:?}, tstpkt0irq: {=bool:?}, dcrpdi: {=bool:?}, confen: {=bool:?}, mcpen: {=bool:?}, hashen: {=bool:?}, ciphen: {=bool:?}, hasdis: {=bool:?}, cipdis: {=bool:?}, clkgat: {=bool:?}, sftrst: {=bool:?} }}" , self . inten () , self . tstpkt0irq () , self . dcrpdi () , self . confen () , self . mcpen () , self . hashen () , self . ciphen () , self . hasdis () , self . cipdis () , self . clkgat () , self . sftrst ())
        }
    }
    #[doc = "Status Registers."]
    #[repr(transparent)]
