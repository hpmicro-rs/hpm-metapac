...
and dac_out."]
    ...
    impl defmt::Format for Cfg {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Cfg {{ fltlen: {=u16:?}, syncen: {=bool:?}, fltmode: {=u8:?}, opol: {=bool:?}, winen: {=bool:?}, fltbyps: {=bool:?}, cmpoen: {=bool:?}, pinsel: {=u8:?}, minsel: {=u8:?}, cmpen: {=bool:?}, hpmode: {=bool:?}, dacen: {=bool:?}, hyst: {=u8:?} }}" , self . fltlen () , self . syncen () , self . fltmode () , self . opol () , self . winen () , self . fltbyps () , self . cmpoen () , self . pinsel () , self . minsel () , self . cmpen () , self . hpmode () , self . dacen () , self . hyst ())
        }
    }
    #[doc = "DAC configure register."]
    #[repr(transparent)]
