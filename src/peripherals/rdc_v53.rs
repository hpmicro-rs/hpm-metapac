...
x man / 2^exp."]
    ...
    impl defmt::Format for RdcCtl {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "RdcCtl {{ exc_en: {=bool:?}, exc_start: {=bool:?}, acc_en: {=bool:?}, rectify_sel: {=u8:?}, acc_len: {=u8:?}, ts_sel: {=u8:?} }}" , self . exc_en () , self . exc_start () , self . acc_en () , self . rectify_sel () , self . acc_len () , self . ts_sel ())
        }
    }
    #[doc = "delay in clock cycle between excitation synchrnous signal and rising edge of i_channel data."]
    #[repr(transparent)]
    ...
    impl core::fmt::Debug for ThrsI {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("ThrsI").field("thrs", &self.thrs()).finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for ThrsI {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "ThrsI {{ thrs: {=u32:?} }}", self.thrs())
        }
    }
    #[doc = "the offset setting for edge detection of the q_channel."]
    #[repr(transparent)]
    ...
    impl core::fmt::Debug for ThrsQ {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("ThrsQ").field("thrs", &self.thrs()).finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for ThrsQ {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "ThrsQ {{ thrs: {=u32:?} }}", self.thrs())
        }
    }
    #[doc = "Configuration for trigger out 0 in clock cycle."]
    #[repr(transparent)]
